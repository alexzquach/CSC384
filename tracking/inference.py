...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        all_possible_probs = util.Counter()

        # Sum up the probabilities of reaching a particular new_position from each old position to get
        # new probabilities.
        for old_position in self.legalPositions:
            ...
            for new_position, probability in new_position_distance.items():
                # Old position and the probability of getting that old position, with the new probability
                all_possible_probs[new_position] += self.beliefs[old_position] * probability

        # NORMALIZE OUR NEW FOUND PROBABILITIES
        all_possible_probs.normalize()
        self.beliefs = all_possible_probs
        "*** END YOUR CODE HERE ***"
